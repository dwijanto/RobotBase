...
uint8_t nrf24l01p::rxMode(uint8_t initialFreq){
	...
	writeRegister(EN_AA, 0x00);			//Disable auto acknowloedment
	writeRegister(EN_RXADDR, 0x01);		//Enable all data pipe (P0 to P5)
	...
	writeRegister(FIFO_STATUS, 0x00);	//Nothing useful to write command
	//const uint8_t* rx_tx_addr = reinterpret_cast<const uint8_t *>("\x66\x88\x68\x68\x68"); // V2x2
	const uint8_t*	rx_tx_addr = reinterpret_cast<const uint8_t *>("\x6D\x6A\x73\x73\x73"); //MJX
